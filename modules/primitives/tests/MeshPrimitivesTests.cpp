...
//
// Layout (ordered by attribute location):
...
static constexpr std::size_t STRIDE        = 32;
...
static glm::vec3 readVec3(const CPUMesh &mesh, std::size_t vertexIndex, std::size_t byteOffset) {
    glm::vec3 v{};
    const auto &raw = mesh.rawData();
    std::memcpy(&v, raw.data() + vertexIndex * STRIDE + byteOffset, sizeof(v));
    return v;
}

static glm::vec2 readVec2(const CPUMesh &mesh, std::size_t vertexIndex, std::size_t byteOffset) {
    glm::vec2 v{};
    const auto &raw = mesh.rawData();
    std::memcpy(&v, raw.data() + vertexIndex * STRIDE + byteOffset, sizeof(v));
    return v;
...
TEST_CASE("makeBox: all normals are unit length", "[primitives]") {
    ...
    for (std::size_t i = 0; i < mesh.vertexCount(); ++i) {
        const glm::vec3 n = readVec3(mesh, i, OFFSET_NORMAL);
        CHECK(glm::length(n) == Catch::Approx(1.0f).epsilon(1e-5f));
    }
}

TEST_CASE("makeBox: UVs are in [0,1] range (scaled by face edge lengths)", "[primitives]") {
    // Unit cube: edge lengths are 1, so UVs should be in [0,1].
    const auto mesh = makeBox({1.0f, 1.0f, 1.0f});
    for (std::size_t i = 0; i < mesh.vertexCount(); ++i) {
        const glm::vec2 uv = readVec2(mesh, i, OFFSET_TEXCOORD);
        CHECK(uv.x >= 0.0f);
        CHECK(uv.y >= 0.0f);
    }
}

TEST_CASE("makeBox: stride matches expected layout", "[primitives]") {
    const auto mesh = makeBox({1.0f, 1.0f, 1.0f});
    CHECK(mesh.layout().getStride() == STRIDE);
}
